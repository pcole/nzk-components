...
function reducer (state, action) {
  switch (action.type) {
    ...
    case 'REMOVE_FIELD': {
      ...
      _sections[action.payload.sectionIndex].fields.splice(
        action.payload.fieldIndex,
        1
      )

      /* eslint-disable */
      _sections[action.payload.sectionIndex].fields = _sections[
    ...
    case 'SET_FIELD_VALUE': {
      ...
      /* eslint-disable */
      _sections3[action.payload.sectionIndex].fields[
        action.payload.fieldIndex
      ].value =
    ...
    case 'CLEAR': {
      ...
      _sections4.forEach(function (section) {
        var fields = []
        for (var _i = 0, len = section.fields.length; _i < len; _i++) {
          fields.push({ value: '' })
        }
    ...
    case 'RESET': {
      return _extends({}, _store.defaultState)
    }
    default:
      return state
  }
